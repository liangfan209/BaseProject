...
//    RelativeLayout mRltQian;
    ...
    @BindView(R2.id.rlt_pay_type)
    RelativeLayout mRltPayType;
...
//        mTvTotalPrice.setText("¥"+AppUtils.getDouble2(mProductInfo.getRealPrice() * mProductInfo.getCount()));
        ...
        if(mProductInfo.getSpecification_list().size()>0){
            ...
            if("full_payment".equals(mProductInfo.getPay_services())){
                ...
                mRltPayType.setVisibility(View.GONE);
            }else if("installment".equals(mProductInfo.getPay_services())){
                //分期
                mRltPayType.setVisibility(View.VISIBLE);
                //首付款
                mTvPayType.setText("¥"+AppUtils.getDouble2(mProductInfo.getDeposit()));
                //剩余尾款
                mTvPayMoney.setText("¥"+AppUtils.getDouble2(realPrice-mProductInfo.getDeposit()));
