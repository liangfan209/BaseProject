...
import com.clkj.wallet.R2;
import com.clkj.wallet.api.bean.BankCard;
import com.clkj.wallet.mvp.wallet.presenter.BankPresenter;
import com.clkj.wallet.mvp.wallet.presenter.WalletPresenter;
...
public class TackCashActivity extends BaseActivity implements WalletIView {
...
//                cbBank.setText(item.getBank_name()+String.format(" (*****%s)",bankNumber.substring(bankNumber.length()-4)));
                cbBank.setText(item.getBank_name()+" ("+bankNumber+")");
    ...
    public void onViewClicked(View view) {
        if (view.getId() == R.id.tv_add_bankcard) {
            ...
            String bankId = adpater.getData().get(mPosition).getId();
            mWalletPresenter.tackCash(moneyInt+"",bankId);
        }
    }
